...
static uintptr_t ReleaseSemaphoreRoutine(void* arg) {
  platform::Semaphore* semaphore = static_cast<platform::Semaphore*>(arg);
  semaphore->Release();
  return 0;
}

struct ReleaseWaitArg {
  platform::Semaphore release_semaphore;
  platform::Semaphore wait_semaphore;
...
TEST(BasicThreadPoolTest, StartThreadPoolTest) {
  volatile uint32_t num = 0;
  platform::Semaphore semaphore(0, 1);
  IncrementArg arg_data = { &num };
  ContainedThreadPool<2, 10> thread_pool;
...
TEST(BasicThreadPoolTest, StartStopFailTest) {
  ...
  platform::Semaphore semaphore1(0, 1);
  platform::Semaphore semaphore2(0, 1);
